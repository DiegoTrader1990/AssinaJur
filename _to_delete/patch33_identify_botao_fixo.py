...
exigir(ancora in src, "ancora nao encontrada (bloco IDENTIFY)")

novo = """        {/* ETAPA 1: Identificação de CPF do Cliente */}
        {step === 'IDENTIFY' && (
          <>
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-2xl space-y-6 pb-28">
              <div className="text-center space-y-2">
                <div className="w-12 h-12 bg-blue-50 border border-blue-200 text-blue-600 rounded-2xl flex items-center justify-center mx-auto shadow-xs">
                  <FileText className="w-6 h-6" />
                </div>
...
print(f"OK patch33 aplicado (tamanho {orig_len} -> {len(src)})")
