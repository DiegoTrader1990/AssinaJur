...
# 1) Remove o aviso comprido sobreposto ao vídeo (banner com 2 linhas de texto) -
# vira só um rótulo curto, e o botão "Tirar foto" passa a ficar logo abaixo dele,
# dentro da área da câmera, em vez de descer até o rodapé da tela.
...
# 2) O status curto + o botão "Tirar foto" ficam juntos, logo abaixo da imagem da
# câmera (fora do vídeo, mas colados a ele) - bem mais perto um do outro.
...
# 3) Remove o bloco antigo do rodapé (aviso oculto + botão "Tirar foto" duplicado)
# - o botão já está junto do status, mais acima.
...
# 4) O contêiner que envolve vídeo + status/botão precisa ser coluna (senão os dois
# blocos ficam lado a lado em vez de um embaixo do outro).
