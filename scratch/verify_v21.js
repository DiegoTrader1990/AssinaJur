async function verifyV21Deployment() {
  for (let attempt = 1; attempt <= 20; attempt++) {
    try {
      ...
      if (chunks[0]) {
        ...
        if (chunkCode.includes('Seu escritório está sob controle') && chunkCode.includes('Iniciar Atendimento')) {
          console.log(`[Attempt ${attempt}] ✅ V21 DEPLOYMENT IS 100% CONFIRMED & LIVE!`);
          return true;
        }
      }
      console.log(`[Attempt ${attempt}] Still building on Vercel... waiting 4s`);
    } catch (e) {
...
verifyV21Deployment().catch(console.error);
