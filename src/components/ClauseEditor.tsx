...
import { Bold, ChevronDown, ChevronUp, GripVertical, Plus, Trash2 } from 'lucide-react';

type ClauseType = 'title' | 'heading' | 'paragraph';
type Alignment = 'left' | 'center' | 'right' | 'justify';
type Clause = { id: string; type: ClauseType; text: string; font: string; size: string; align: Alignment; bold: boolean; spaceAfter: 'normal' | 'large' };
const makeClause = (type: ClauseType, text = ''): Clause => ({ id: crypto.randomUUID(), type, text, font: 'Helvetica', size: type === 'title' ? '4' : '3', align: type === 'title' ? 'center' : type === 'paragraph' ? 'justify' : 'left', bold: type !== 'paragraph', spaceAfter: 'normal' });

function toClauses(html: string): Clause[] {
  const root = document.createElement('div'); root.innerHTML = html;
  const blocks = Array.from(root.children).flatMap((node) => {
    const element = node as HTMLElement; const text = (element.innerText || element.textContent || '').replace(/\[\[AJ_BR\]\]/g, '\n').replace(/\u00a0/g, ' ').replace(/[^\S\r\n]+/g, ' ').trim();
    if (!text) return [];
    ...
    clause.bold = /data-aj-bold\s*=\s*["']true["']/i.test(element.outerHTML) || clause.bold;
  ...
  const raw = root.textContent?.replace(/\[\[AJ_BR\]\]/g, '\n').replace(/\u00a0/g, ' ').replace(/[^\S\r\n]+/g, ' ').trim() || '';
...
function clauseToHtml(clause: Clause) {
  const raw = escapeHtml(clause.text).replace(/\r?\n/g, '[[AJ_BR]]');
  const content = `<font face="${clause.font}" size="${clause.size}">${clause.bold ? `<strong>${raw}</strong>` : raw}</font>`;
  ...
  return `<${tag} data-aj-bold="${clause.bold}" style="text-align: ${clause.align};">${body}</${tag}>${clause.spaceAfter === 'large' ? '<p data-aj-spacer="large">&nbsp;</p>' : ''}`;
}
