...
function clientOperational(client: CentralClient) {
  ...
  const urgent = ['URGENTE', 'ALTA'].includes(mainPendency?.priority || '') || processDue?.priority === 'ALTA';
  ...
  return {
    ...
    newThisMonth,
    awaitingDocs,
    movementCount: client.movementSummary?.total || 0,
    openAlerts: client.movementSummary?.openAlerts ?? pendencies.length,
    activeProcessCount: client.movementSummary?.activeProcesses ?? activeProcesses.length,
    documentCount: client.movementSummary?.documents ?? (client.documents || []).length,
...
const ALERT_PRIORITY: Record<string, { label: string; classes: string; dot: string; stripe: string }> = {
  URGENTE: { label: 'Alerta urgente', classes: 'border-rose-200 bg-rose-50 text-rose-700', dot: 'bg-rose-500', stripe: 'bg-rose-500' },
  ALTA: { label: 'Prioridade alta', classes: 'border-orange-200 bg-orange-50 text-orange-700', dot: 'bg-orange-500', stripe: 'bg-orange-500' },
  NORMAL: { label: 'Prioridade normal', classes: 'border-[#eadca9] bg-[#fffaf0] text-[#8a6810]', dot: 'bg-[#c39a25]', stripe: 'bg-[#c39a25]' },
  BAIXA: { label: 'Prioridade baixa', classes: 'border-slate-200 bg-slate-50 text-slate-600', dot: 'bg-slate-400', stripe: 'bg-slate-400' },
};
...
export default function ClientsCentral({
  ...
  onCreateFollowUp: (client: CentralClient, pendency?: Pendency) => void;
}) {
  ...
  const metrics = useMemo(() => ({
    ...
    openAlerts: enriched.reduce((total, item) => total + item.info.openAlerts, 0),
    activeProcesses: enriched.reduce((total, item) => total + item.info.activeProcessCount, 0),
    movements: enriched.reduce((total, item) => total + item.info.movementCount, 0),
              ...
              {visible.map(({ client, info }) => {
                ...
                return (
                  <article key={client.id} onClick={() => onOpen(client)} className="group relative cursor-pointer rounded-[18px] border border-slate-200 bg-white p-3.5 shadow-[0_10px_28px_-28px_rgba(7,27,58,.48)] transition duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_16px_38px_-26px_rgba(7,27,58,.34)]">
                    <span className={`absolute inset-y-4 left-0 w-[3px] rounded-r-full ${alertInfo?.stripe || statusInfo.dot}`} />
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="relative shrink-0" onClick={(event) => event.stopPropagation()}><button onClick={() => setMenuId(menuId === client.id ? null : client.id)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-transparent text-slate-400 transition hover:border-slate-200 hover:bg-slate-50 hover:text-[#071B3A]" aria-label={`Mais ações para ${client.name}`}><MoreHorizontal className="h-4 w-4" /></button>{menuId === client.id && <ActionMenu menuRef={menuRef} client={client} hasAlert={Boolean(info.mainPendency)} onOpen={() => onOpen(client)} onEdit={() => onEdit(client)} onFollowUp={() => onCreateFollowUp(client, info.mainPendency)} onDelete={() => onDelete(client)} />}</div>
                        </div>
                        <div className="mt-2 flex flex-wrap items-center gap-1.5"><span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-[8px] font-black ${alertInfo?.classes || statusInfo.classes}`}><span className={`h-1.5 w-1.5 rounded-full ${alertInfo?.dot || statusInfo.dot}`} />{alertInfo?.label || statusInfo.label}</span>{info.pendingDocuments.length > 0 && <span className="rounded-full border border-sky-100 bg-sky-50 px-2 py-1 text-[8px] font-black text-sky-700">Assinatura pendente</span>}</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 rounded-xl border border-[#eadca9] bg-[#fffdf7] px-2.5 py-2">
                      <div className="flex items-start gap-2"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-lg bg-[#f4e9bd] text-[#8c6910]"><Zap className="h-3 w-3" /></span><div className="min-w-0 flex-1"><span className="block text-[8px] font-black uppercase tracking-[.11em] text-[#9c7b24]">{info.mainPendency ? 'Alerta e próxima ação' : 'Próxima ação'}</span><strong className="mt-0.5 block truncate text-[10px] font-black leading-4 text-[#071B3A]">{info.nextAction}</strong>{due && <span className={`mt-0.5 inline-flex items-center gap-1 text-[8px] font-bold ${due.overdue ? 'text-rose-600' : 'text-amber-700'}`}><CalendarClock className="h-3 w-3" />{due.text}</span>}</div>{info.mainPendency && <button onClick={(event) => { event.stopPropagation(); onCreateFollowUp(client, info.mainPendency); }} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-[#dec66e] bg-white text-[#8c6910] transition hover:bg-[#f8edc4]" aria-label="Editar alerta"><Palette className="h-3.5 w-3.5" /></button>}</div>
                    </div>

                    <div className="mt-2 flex items-center gap-1.5 overflow-hidden text-[8px] font-bold text-slate-500">
                      <span className="rounded-lg bg-slate-50 px-2 py-1">{client.movementSummary?.processes || 0} processos</span>
                      <span className="rounded-lg bg-slate-50 px-2 py-1">{info.documentCount} docs</span>
                      <span className="rounded-lg bg-slate-50 px-2 py-1">{info.openAlerts} alertas</span>
                      <span className="ml-auto whitespace-nowrap rounded-lg bg-[#071B3A] px-2 py-1 text-white">{info.movementCount} mov.</span>
                    </div>
                    ...
                    <div className="mt-2.5 flex items-center justify-between gap-2">
                      ...
                      <div className="flex shrink-0 items-center gap-1" onClick={(event) => event.stopPropagation()}>{whatsapp && <a href={`https://wa.me/${whatsapp.startsWith('55') ? whatsapp : `55${whatsapp}`}`} target="_blank" rel="noreferrer" className="flex h-8 w-8 items-center justify-center rounded-lg border border-emerald-100 bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" aria-label={`WhatsApp de ${client.name}`}><MessageCircle className="h-3.5 w-3.5" /></a>}<button onClick={() => onCreateFollowUp(client, info.mainPendency)} className="inline-flex h-8 items-center gap-1 rounded-lg border border-[#dec66e] bg-[#fffaf0] px-2 text-[8px] font-black text-[#7d5f0d] transition hover:bg-[#f8edc4]">{info.mainPendency ? <Palette className="h-3 w-3" /> : <Plus className="h-3 w-3" />} {info.mainPendency ? 'Gerenciar' : 'Alerta'}</button><button onClick={() => onOpen(client)} className="h-8 rounded-lg bg-[#071B3A] px-2.5 text-[8px] font-black text-white transition hover:bg-[#12335e]">Abrir ficha</button></div>
                    </div>
                  </article>
                );
              })}
...
function ActionMenu({ menuRef, client, hasAlert, onOpen, onEdit, onFollowUp, onDelete }: { menuRef: React.RefObject<HTMLDivElement>; client: CentralClient; hasAlert: boolean; onOpen: () => void; onEdit: () => void; onFollowUp: () => void; onDelete: () => void }) {
  ...
  return <div ref={menuRef} className="absolute right-0 top-10 z-30 w-56 rounded-2xl border border-slate-200 bg-white p-2 text-left shadow-[0_18px_45px_-15px_rgba(15,23,42,.28)]">
    ...
    <button onClick={onFollowUp} className={item}>{hasAlert ? <Palette className="h-3.5 w-3.5 text-[#9c7b24]" /> : <CalendarClock className="h-3.5 w-3.5 text-[#9c7b24]" />} {hasAlert ? 'Gerenciar alerta e cor' : 'Criar alerta e acompanhamento'}</button>
