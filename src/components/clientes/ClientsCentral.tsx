...
export default function ClientsCentral({
  ...
  return (
    <div className="space-y-5">
      <section className="relative overflow-hidden rounded-[28px] border border-[#15345f] bg-[#071B3A] px-5 py-6 text-white shadow-[0_18px_50px_-30px_rgba(7,27,58,.8)] sm:px-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-3 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.22em] text-[#e4c76c]"><Sparkles className="h-3.5 w-3.5" /> Gestão de relacionamento jurídico</div>
            <h1 className="font-heading text-2xl font-black tracking-tight sm:text-[30px]">Central de Clientes</h1>
            <p className="mt-2 max-w-xl text-xs font-medium leading-5 text-slate-300 sm:text-sm">Gerencie dados, documentos, demandas e acompanhamentos dos clientes do escritório em uma visão operacional.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={onRefresh} className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/15 bg-white/[.06] px-3.5 text-xs font-bold text-slate-100 transition hover:bg-white/10"><Clock3 className="h-3.5 w-3.5" /> Atualizar</button>
            <button onClick={onCreate} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#d6b23f] px-4 text-xs font-black text-[#071B3A] shadow-lg shadow-black/15 transition hover:bg-[#e5c659]"><UserPlus className="h-4 w-4" /> Novo Cliente</button>
          </div>
        </div>
      </section>

      <section className="grid grid-cols-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:grid-cols-3 xl:grid-cols-6">
        {metricItems.map(({ label, value, Icon, color, quick }) => (
          <button key={label} onClick={() => quick && setQuickFilter(quick)} className="group flex min-h-[92px] items-center gap-3 border-b border-r border-slate-100 px-4 text-left transition hover:bg-slate-50 xl:border-b-0">
            <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-50 ${color}`}><Icon className="h-4 w-4" /></span>
            <span><strong className="block font-heading text-lg font-black text-[#071B3A]">{value}</strong><span className="mt-0.5 block text-[9px] font-bold uppercase leading-3 tracking-wide text-slate-400">{label}</span></span>
          </button>
        ))}
      </section>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1120px] table-fixed text-left">
              <thead className="sticky top-0 z-10 border-b border-slate-200 bg-slate-50/95 text-[9px] font-black uppercase tracking-[.11em] text-slate-400 backdrop-blur">
                <tr><th className="w-11 px-4 py-3"><input type="checkbox" checked={visible.length > 0 && visible.every(({ client }) => selected.has(client.id))} onChange={toggleAllVisible} className="h-4 w-4 rounded border-slate-300 text-blue-600" /></th><th className="w-[23%] px-2 py-3">Cliente</th><th className="w-[17%] px-3 py-3">Contato</th><th className="w-[18%] px-3 py-3">Área / Demanda</th><th className="w-[14%] px-3 py-3">Status</th><th className="w-[19%] px-3 py-3">Pendência / Próxima ação</th><th className="w-[14%] px-3 py-3">Última atividade</th><th className="w-28 px-4 py-3 text-right">Ações</th></tr>
              </thead>
                ...
                {visible.map(({ client, info }) => {
                  ...
                  return (
                    <tr key={client.id} onClick={() => onOpen(client)} className="group cursor-pointer bg-white transition hover:bg-[#f8fafc]">
                      <td className="px-4 py-4" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={selected.has(client.id)} onChange={() => setSelected((current) => { const next = new Set(current); next.has(client.id) ? next.delete(client.id) : next.add(client.id); return next; })} className="h-4 w-4 rounded border-slate-300 text-blue-600" /></td>
                      <td className="px-2 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-slate-100 text-[11px] font-black text-[#071B3A]">{initials(client.name)}</span><span className="min-w-0"><strong className="block truncate font-heading text-xs font-black text-[#071B3A] group-hover:text-blue-700">{client.name}</strong><span className="mt-1 block truncate text-[10px] font-semibold text-slate-400">{maskCpfCnpj(client.cpfCnpj)}{client.city ? ` · ${client.city}${client.state ? `/${client.state}` : ''}` : ''}</span></span></div></td>
                      <td className="px-3 py-4"><div className="flex items-center gap-2"><span className="min-w-0"><strong className="block truncate text-[11px] font-extrabold text-slate-700">{maskPhone(client.phone)}</strong><span className="mt-1 block truncate text-[10px] text-slate-400">{client.email || 'E-mail não informado'}</span></span>{whatsapp && <a href={`https://wa.me/${whatsapp.startsWith('55') ? whatsapp : `55${whatsapp}`}`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="ml-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" aria-label={`WhatsApp de ${client.name}`}><MessageCircle className="h-3.5 w-3.5" /></a>}</div></td>
                      <td className="px-3 py-4"><strong className="block truncate text-[11px] font-extrabold text-[#071B3A]">{client.legalArea || info.currentProcess?.legalArea || 'Geral'}{info.currentProcess?.title ? ` · ${info.currentProcess.title}` : ''}</strong><div className="mt-1.5 flex flex-wrap gap-1">{info.currentProcess?.protocolNumber && <span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[8px] font-bold text-slate-500">Protocolo {info.currentProcess.protocolNumber}</span>}{info.pendingDocuments.length > 0 && <span className="rounded-md bg-sky-50 px-1.5 py-0.5 text-[8px] font-bold text-sky-700">Assinatura pendente</span>}</div></td>
                      <td className="px-3 py-4"><span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-black ${statusInfo.classes}`}><span className={`h-1.5 w-1.5 rounded-full ${statusInfo.dot}`} />{statusInfo.label}</span>{client.lawyerInCharge && <span className="mt-1.5 block truncate text-[9px] font-semibold text-slate-400">Resp. {client.lawyerInCharge.name}</span>}</td>
                      <td className="px-3 py-4"><strong className="line-clamp-2 text-[11px] font-extrabold leading-4 text-slate-700">{info.nextAction}</strong>{due && <span className={`mt-1 inline-flex items-center gap-1 text-[9px] font-bold ${due.overdue ? 'text-rose-600' : 'text-amber-700'}`}><CalendarClock className="h-3 w-3" />{due.text}</span>}</td>
                      <td className="px-3 py-4"><strong className="block text-[10px] font-extrabold text-slate-600">{dateLabel(info.lastActivity)}</strong><span className="mt-1 block truncate text-[9px] text-slate-400">{info.activityDescription}</span></td>
                      <td className="px-4 py-4 text-right" onClick={(event) => event.stopPropagation()}><div className="relative inline-flex items-center gap-1"><button onClick={() => onOpen(client)} className="rounded-lg bg-[#071B3A] px-3 py-2 text-[9px] font-black text-white transition hover:bg-[#12335e]">Ver cliente</button><button onClick={() => setMenuId(menuId === client.id ? null : client.id)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:bg-slate-50"><MoreHorizontal className="h-4 w-4" /></button>{menuId === client.id && <ActionMenu menuRef={menuRef} client={client} onOpen={() => onOpen(client)} onEdit={() => onEdit(client)} onDelete={() => onDelete(client)} />}</div></td>
                    </tr>
                  );
                })}
              </tbody>
