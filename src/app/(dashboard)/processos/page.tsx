            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  {selected.driveFolderUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-[10px] font-extrabold text-blue-700 inline-flex gap-1 items-center"
                    >
                      <HardDrive className="w-3 h-3" /> Abrir Drive{" "}
                ...
                <div
                  ...
                  className={`mt-2 rounded-2xl border-2 border-dashed p-5 text-center transition-colors ${draggingFiles ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-slate-50"}`}
                >
                  <CloudUpload
                    className={`w-7 h-7 mx-auto ${draggingFiles ? "text-blue-600" : "text-slate-400"}`}
                  />
                  <p className="mt-2 text-xs font-bold text-[#071B3A]">
                    Arraste PDFs aqui
                  </p>
                  <p className="text-[11px] text-slate-500 mt-1">
                    ou selecione vários arquivos do computador
                  </p>
                  <label className="mt-3 inline-flex cursor-pointer text-[11px] font-extrabold text-blue-700 border border-blue-200 rounded-lg px-3 py-2 bg-white gap-1 items-center">
                    <Upload className="w-3.5 h-3.5" />
                    {uploadingAttachment ? "Enviando..." : "Selecionar PDFs"}
                    <input
                      type="file"
                      multiple
                ...
                <div className="mt-2 space-y-2">
                  {selected.attachments.length ? (
                    selected.attachments.map((a) => (
                      <div
                        key={a.id}
                        className="border rounded-xl p-3 flex justify-between items-center gap-2 text-sm"
                      >
                        ...
                        <div className="flex gap-2 shrink-0">
                          <a
                            href={`/api/documents/upload?fileId=${a.file.id}`}
                            download
                            className="text-emerald-700 font-bold text-xs flex items-center gap-1"
                          >
                    ...
                    <p className="text-sm text-slate-500">
                      Anexe PDFs como protocolos, laudos e petições.
                    </p>
                  )}
                </div>
              </div>
